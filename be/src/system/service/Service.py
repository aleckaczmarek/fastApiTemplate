...
class Service():
    ...
    async def delete(self, item,middleware: Optional[Callable[..., Awaitable[T]]]):
       async def delete(data):
            if type(data) == Data:
                   return await self.repo.delete(data.data.Id)
            else:
               return await self.repo.delete(data.Id)
            
       result = await middlewareRunner(item,delete,middleware)
       return result
